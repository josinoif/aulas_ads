...
> **Shell:** Bash (Linux ou **Git Bash** no Windows) — `bash scripts/e2e-prepare.sh` a partir de `backend/nest/`.
...
A partir de `backend/nest/` — **Pare** `npm run start:dev` antes.

```bash
# a partir de backend/nest/:
bash scripts/e2e-prepare.sh
# se estiver em loja-api/:
# bash ../scripts/e2e-prepare.sh
```

O script sobe Postgres, sincroniza schema se `users` ainda não existir e aplica o seed (`DELETE` + `ALTER SEQUENCE`). Preferível a partir de **`backend/nest/`**; de `loja-api/` use `bash ../scripts/e2e-prepare.sh` (o script resolve Compose/seed).
...
> **Nest 12 CJS + Jest 30:** o scaffold usa Jest 30. Nos scripts `test` / `test:e2e` do `package.json`, rode o Jest com a flag `--experimental-vm-modules` (pacotes `@nestjs/*` são ESM; o app continua CJS):
>
> ```json
...
> ```
>
> Se o scaffold antigo ainda tiver só `jest`, atualize esses dois scripts.
...
> **Avançado (opcional, 2 linhas):** `globalSetup` do Jest ou `npm run test:e2e:prepare` que **só chama** o script acima — não invente outro fluxo. Script: [`e2e-prepare.sh`](scripts/e2e-prepare.sh).
...
O prepare do §3 (`e2e-prepare.sh`) deixa Ana/Cli e o catálogo prontos antes desta suíte.
...
- `package.json` (scripts `test` / `test:e2e` com `--experimental-vm-modules`) · `src/products/products.service.spec.ts` (unitário) · `test/*.e2e-spec.ts` · `test/jest-e2e.json` · (opcional) `globalSetup` — não é obrigatório na linha P
