...
Compose: [`docker-compose.postgres.yml`](docker-compose.postgres.yml). Seed: [`seed/seed.sql`](seed/seed.sql) + [`seed/verify-seed.sh`](seed/verify-seed.sh).
...
**Script pronto no material** — [`scripts/e2e-prepare.sh`](scripts/e2e-prepare.sh). A partir de `backend/nest/` (sobe Postgres, **sincroniza schema** se `users` ainda não existir, aplica seed):
...
**Pré-requisito:** rode o [`seed.sql`](seed/seed.sql) do §3 (ou `globalSetup` / `npm run test:e2e:prepare`) **antes** desta suíte. O exemplo abaixo **só faz login** — assume Ana já no Postgres.
