...
Na pasta `backend/nest/` há um Compose de referência: [`docker-compose.postgres.yml`](docker-compose.postgres.yml).
...
Instale no `loja-api` (você fará de novo no cap. 5 se ainda não instalou). **Use os pins** — o TypeORM “latest” no npm já é a linha **1.x**, que exige Node mais novo e não é o alvo desta trilha:

| Pacote | Versão da trilha | Nota |
|--------|------------------|------|
| `@nestjs/typeorm` | **10** | integração Nest ↔ TypeORM |
| `typeorm` | **0.3** | ORM (não instale 1.x aqui) |
| `pg` | **8** | driver oficial do PostgreSQL no Node |
