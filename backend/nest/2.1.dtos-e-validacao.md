...
> **Só aula / vídeo:** §1–4 + [§4.1](#41-linha-p--plugar-dtos-no-controller-agora). O resto (§5–7) é **consulta** — não precisa ler tudo hoje.  
> **Recuperação (30s):** o que `@Body()` entrega? O CRUD do cap. 2 valida o JSON?  
> **Neste episódio:** (1) tipagem ≠ validação · (2) DTO classe + pipe · (3) `POST` inválido → 400.  
> **Pause e rode:** depois do pipe, mande `price:"barato"` e veja o 400.  
> **Windows:** `curl.exe` nos curls de evidência; status: `-o NUL -w "%{http_code}\n"` — [FAQ](FAQ-TRAVOU.md).

## Objetivo deste material

Ao terminar a **linha P**, você deve conseguir:

1. Explicar o que `@Body()` entrega (e o que **não** garante sozinho).
2. Modelar o corpo com **DTOs** (classes) + `class-validator` / `class-transformer`.
3. Ativar o **`ValidationPipe`** global (`whitelist`, `forbidNonWhitelisted`, `transform`).
4. Proteger `POST` / `PUT` `/products` (mesmo contrato do create no update).

**Linha A / consulta (não bloqueia o próximo capítulo):** mensagens customizadas, `PATCH` + `PartialType`, query DTO, catálogo de decorators (§5–7).
...
> **Nesta aula da loja:** proteger `POST`/`PUT` `/products` — Ana não pode cadastrar Caneca Nest com `price: "barato"` ou campos extras. Domínio âncora: **Product**.  
...
> **Consulta rápida** (pipes + lembrete de validators): [REFERENCIA-NEST §2](REFERENCIA-NEST.md#sec-pipes). Tabelas longas de `class-validator` ficam **neste** capítulo (§6).

### Pacotes

Estas duas bibliotecas **não** vêm no scaffold do Nest. Instale-as com as versões da trilha (compatíveis com Nest 10 e com o `ValidationPipe`):

| Pacote | Versão da trilha | Papel |
|--------|------------------|--------|
| `class-validator` | **0.14** | regras (`@IsString`, `@Min`, …) |
| `class-transformer` | **0.5** | plain JSON → instância da classe DTO |
...
> Se o npm oferecer uma major mais nova (ex.: `class-validator@0.15`), **não** atualize sozinho nesta trilha — o pin acima é o que foi testado com Nest 10.
...
## ⛔ FIM DA AULA / VÍDEO (linha P)

Você já fechou o essencial: **DTO classe** + **`ValidationPipe` global** + **`POST` inválido → 400**.  
Pode ir para [Services](3.services.md). O que vem abaixo (§5–7) é **consulta** — não leia de ponta a ponta hoje.

------

## Apêndice — consulta rápida (seções 5–7)

> As seções abaixo aprofundam *por quê* e *quais* decorators existem. Na aula, foque em ligar o pipe + `CreateProductDto` e o curl `400` acima; volte aqui quando precisar de um decorator específico. Catálogo por objetivo: [REFERENCIA-NEST §2](REFERENCIA-NEST.md#sec-pipes).
...
`PartialType` vem do pacote **`@nestjs/mapped-types`**, que **não** é instalado automaticamente no cap. 2.1 (o Swagger só entra no cap. 8). Se for fazer o desafio A agora, instale:

| Pacote | Versão da trilha |
|--------|------------------|
| `@nestjs/mapped-types` | **2** (ex.: 2.0.6 — peer Nest 8–10) |

```bash
npm install @nestjs/mapped-types@2.0.6
```
...
Pipes e validators na linha P: [REFERENCIA-NEST.md §2](REFERENCIA-NEST.md#sec-pipes) · detalhe da aula: tabelas acima.
