...
Estas duas bibliotecas **não** vêm no scaffold do Nest. Instale-as com as versões da trilha (compatíveis com Nest 12 e com o `ValidationPipe`):

| Pacote | Versão da trilha | Papel |
|--------|------------------|--------|
| `class-validator` | **0.14.1** | regras (`@IsString`, `@Min`, …) |
| `class-transformer` | **0.5.1** | plain JSON → instância da classe DTO |
...
> Se o npm oferecer uma major mais nova (ex.: `class-validator@0.15`), **não** atualize sozinho nesta trilha — o pin acima é o que foi testado com Nest 12.
...
```bash
npm install @nestjs/mapped-types@12.0.0
```
