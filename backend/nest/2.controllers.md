...
> **Neste episódio:** (1) controller na borda HTTP · (2) CRUD `/products` em RAM · (3) status `201`/`204`/`404`.  
> **Pause e rode:** após o CRUD, rode os curls da §6 e confira os status.  
> **Recuperação (30s):** o que é `GET /health`? Onde fica o `loja-api`?  
> **Windows:** use `curl.exe` (não o alias `curl` do PowerShell). Travou? [FAQ](FAQ-TRAVOU.md).  
> **Linha P hoje:** só o CRUD em memória. **A** (PATCH) no fim — não bloqueia o 2.1.
...
> **Consulta:** lista de decorators HTTP → [REFERENCIA-NEST §1](REFERENCIA-NEST.md#sec-http).
...
> Na URL tudo é texto. Por isso `Number(id)` neste capítulo. No **cap. 3** (e de novo no 5 com TypeORM) você troca por `@Param('id', ParseIntPipe)` — o pipe fica no [REFERENCIA-NEST §2](REFERENCIA-NEST.md#sec-pipes).
...
> **Shell:** Git Bash / macOS / Linux — blocos abaixo. **PowerShell:** `curl.exe` (não o alias `curl`); status HTTP: `curl.exe … -o NUL -w "%{http_code}\n"` (não `/dev/null`, não `-Method` — isso é `Invoke-WebRequest`). Detalhe: [FAQ](FAQ-TRAVOU.md) · [CURLS-P](CURLS-P.md).
...
- Esqueceu decorator? [REFERENCIA-NEST §1 HTTP](REFERENCIA-NEST.md#sec-http)  
