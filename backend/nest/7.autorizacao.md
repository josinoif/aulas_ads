...
> **Recuperação (30s):** autenticação vs autorização? O que é `sub` no JWT? Sem Bearer em `POST /products` → qual status (cap. 6)?  
> **Neste episódio:** `RolesGuard` + matriz ADMIN/CLIENT; Cli → **403** em mutação de produto.  
> **Pause e rode:** curl do Cli em `POST /products` → `403`; Ana → `201`.  
> **Shell:** Bash (Linux ou **Git Bash** no Windows) — `TOKEN=$(…)` e curls Bearer.
...
```mermaid
flowchart TB
    subgraph papel["RolesGuard — papel"]
        R["@Roles('ADMIN')"]
    end
    subgraph dono["Service — dono do pedido"]
        F["findOneForActor / cancelForActor"]
    end
    Req[Requisição autenticada] --> papel
    papel -->|ADMIN ou CLIENT ok na rota| dono
    dono -->|Cli vê só os próprios| Ok[Handler]
```
...
Seed de Ana e Cli (lab): [`seed/seed.sql`](seed/seed.sql).
...
> **Consulta:** [REFERENCIA-NEST §3 Guards](REFERENCIA-NEST.md#sec-guards) (401 vs 403).
...
(Coloque num controller **sem** rota `@Get(':id')` antes — senão `"demo"` vira id. Ex.: `AppController` ou `DemoController`.)
...
> **Pause intermediário:** `RolesGuard` exportado + `AuthModule` importado em products/orders → **só então** os curls Ana/Cli abaixo.
...
sequenceDiagram
    actor Cli
    participant API
    Cli->>API: POST /products + Bearer CLIENT
    API-->>Cli: 403 Forbidden
    Note over API: autenticado, sem papel ADMIN
```

Extrair token — opção **A** (`python`) | **B** (`jq`) | **C** (manual). Detalhes em [CURLS-P.md](CURLS-P.md).

```bash
# Opção A — python (no Linux, python3 também funciona)
...
- `@Roles` / `RolesGuard` / 401 vs 403: [REFERENCIA-NEST §3](REFERENCIA-NEST.md#sec-guards)  
