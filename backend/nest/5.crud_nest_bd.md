...
> **Recuperação (30s):** por que DTO é **classe** e não interface? O que `whitelist` faz no `ValidationPipe`? O service do cap. 3 ainda usa array — o que muda hoje?  
> **Neste episódio:** Docker Postgres → TypeORM → mesmos curls `/products`, dados sobrevivem ao restart.  
> **Pause e rode:** Compose `healthy` + `psql` responde — **antes** de criar a entidade.  
> **Shell:** Bash (Linux ou **Git Bash** no Windows) — necessário para `docker exec … < seed/…`.  
> **Seed:** `seed-catalog` só no cap. 5. Depois de pedidos **não** reaplicar só o catalog. [`seed.sql`](seed/seed.sql) completo = **cap. 6+** (quando existir `users`).
...
A partir de `backend/nest/` (ou copie o Compose para a raiz do `loja-api`). Arquivo: [`docker-compose.postgres.yml`](docker-compose.postgres.yml).
...
> **Pause — checkpoint intermediário:** `docker compose … ps` mostra Postgres **healthy**? Só então siga para a entidade. Se o nome `loja-postgres` já existir: `docker start loja-postgres` — [FAQ](FAQ-TRAVOU.md).
...
No `loja-api`, instale com as **versões da trilha** (Nest 12). Sem o `@versão`, o npm pode puxar `typeorm@1` (major incompatível) e o lab quebra:

| Pacote | Versão da trilha | Papel |
|--------|------------------|--------|
| `@nestjs/typeorm` | **12.0.1** | módulo TypeORM no Nest |
| `typeorm` | **0.3.31** | ORM (linha 0.3 — **não** use 1.x) |
| `pg` | **8.13.3** | driver PostgreSQL |
| `@nestjs/config` | **12.0.0** | lê o `.env` |

```bash
npm install @nestjs/typeorm@12.0.1 typeorm@0.3.31 pg@8.13.3
npm install @nestjs/config@12.0.0
```

> O `npm install` dos pacotes TypeORM/`pg`/`config` é **só neste capítulo** (o cap. 4 é prévia conceitual — sem install).
> **Aviso:** sem pin, o npm pode puxar **`typeorm@1`** — use exatamente `typeorm@0.3.31`.
> **Conceito: driver `pg`**  
...
Crie `.env` na raiz do `loja-api` (pode copiar de [`.env.example`](.env.example)):
...
Se ainda não usa `@nestjs/config`, pode passar os valores literalmente como no **esboço/prévia** do [cap. 4](4.introducao_nestjs_persistencia.md) — o importante é `type: 'postgres'` e a porta `5432`.
...
> **Checklist antes de subir:** no controller, o `:id` precisa ser **número**. Troque assinaturas assim (padrão do [cap. 3](3.services.md)):
...
Se ainda estiver `id: string` + `Number(id)` só no service, o TypeORM pode falhar ou buscar o id errado.

> **Consulta:** `ParseIntPipe` → [REFERENCIA-NEST §2](REFERENCIA-NEST.md#sec-pipes) · TypeORM → [§5](REFERENCIA-NEST.md#sec-typeorm).
...
Com a API já tendo subido uma vez (`synchronize: true` criou `products`), a partir de `backend/nest/` aplique [`seed/seed-catalog.sql`](seed/seed-catalog.sql):

```bash
# a partir de backend/nest/ — o `<` injeta o arquivo SQL no stdin do psql (só funciona no Bash)
docker exec -i loja-postgres psql -U loja -d loja < seed/seed-catalog.sql
curl -s http://localhost:3000/products
```

> Use [`seed/seed.sql`](seed/seed.sql) (completo, com Ana/Cli) **só no cap. 6+**, depois da API subir com a entidade `User` — ele apaga `orders` / `users` e falha se as tabelas ainda não existirem.  
> **Não** reaplicar [`seed-catalog.sql`](seed/seed-catalog.sql) depois de criar pedidos: ele só mexe em `products` e pode deixar `order_items` órfãos (nesta trilha não há FK Product→OrderItem). No cap. 6+, use o seed completo.
...
TypeORM: [REFERENCIA-NEST §5](REFERENCIA-NEST.md#sec-typeorm) · `ParseIntPipe`: [§2](REFERENCIA-NEST.md#sec-pipes).
