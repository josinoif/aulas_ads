...
> **Windows:** seed com `Get-Content .\seed\seed-catalog.sql | docker exec -i …` — ver [FAQ](FAQ-TRAVOU.md).  
> **Seed:** `seed-catalog` só no cap. 5; depois de pedidos use [`seed.sql`](seed/seed.sql) completo (não reaplicar só o catalog).
...
No `loja-api`, instale com as **versões da trilha** (Nest 10). Sem o `@versão`, o npm pode puxar `@nestjs/config@12` ou `typeorm@1`, incompatíveis com este material:

| Pacote | Versão da trilha | Papel |
|--------|------------------|--------|
| `@nestjs/typeorm` | **10.0.2** | módulo TypeORM no Nest |
| `typeorm` | **0.3.26** | ORM (linha 0.3 — não use 1.x) |
| `pg` | **8.13.3** | driver PostgreSQL |
| `@nestjs/config` | **3.3.0** | lê o `.env` (peer Nest 8–10; a v12 exige Nest 11+) |

```bash
npm install @nestjs/typeorm@10.0.2 typeorm@0.3.26 pg@8.13.3
...
> **Shell:** Git Bash abaixo. **PowerShell:** `curl.exe` · seed: `Get-Content .\seed\seed-catalog.sql | docker exec -i loja-postgres psql -U loja -d loja` — [FAQ](FAQ-TRAVOU.md).
...
> Use [`seed/seed.sql`](seed/seed.sql) (completo, com Ana/Cli) **só depois** dos caps. 5.1 + 6 — ele apaga `orders` / `users` e falha se as tabelas ainda não existirem.  
> **Não** reaplicar [`seed-catalog.sql`](seed/seed-catalog.sql) depois de criar pedidos: ele só mexe em `products` e pode deixar `order_items` órfãos (nesta trilha não há FK Product→OrderItem). Use o seed completo.
