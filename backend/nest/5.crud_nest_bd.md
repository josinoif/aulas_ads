...
> Se você **já** instalou no cap. 4 com os mesmos pins, o segundo `npm install` só confirma as versões — não precisa desinstalar nada.
...
> **Checklist antes de subir:** no controller, o `:id` precisa ser **número** — use `@Param('id', ParseIntPipe) id: number` (como no [cap. 3](3.services.md)). Se ainda estiver `id: string` + `Number(id)` só no service, o TypeORM pode falhar ou buscar o id errado.
