...
> **Neste episódio (vídeo / aula):** (1) por que Nest · (2) `npx @nestjs/cli@10 new loja-api` · (3) `GET /health` → ok.  
...
- **Node.js 20 LTS** (mínimo aceitável: 18). Confira com `node -v` — o ideal nesta trilha é `v20.x`
- **npm** (vem com o Node)
...
```bash
node -v   # esperado: v20.x (ou v18.x)
npm -v
```
...
> **Por que `@10`?** No npm, `@nestjs/cli` “latest” já é a linha **12**. Nest 12 muda scaffold (ESM), exige Node mais novo e **não** combina com os pacotes pinados desta trilha. Por isso criamos o projeto **sempre** com a CLI 10.

```bash
cd backend/nest          # pasta deste material
# Versão da trilha: NestJS 10 (CLI 10.x — ex.: 10.4.9)
