...
> **Neste episódio (vídeo / aula):** (1) por que Nest · (2) scaffold Nest 12 CJS · (3) `GET /health` → ok.  
> **Pause e rode:** após `npm run start:dev`, confirme o health antes de seguir.  
> **Shell:** Bash (Linux ou **Git Bash** no Windows). Instale [Git for Windows](https://git-scm.com/download/win) se ainda não tiver. No Windows: **Menu Iniciar → Git Bash** (não use o terminal PowerShell do Cursor/VS Code). Travou? [FAQ](FAQ-TRAVOU.md).
...
- **Node.js 20.19+** ou **22.12+** (Nest 12; **não** use 21.x). Confira com `node -v`
- **npm** (vem com o Node)
- Editor (VS Code / Cursor)
- **Bash:** no Linux, o terminal normal; no Windows, **Git Bash** ([Git for Windows](https://git-scm.com/download/win)) — abra pelo Menu Iniciar (o terminal integrado do Cursor/VS Code costuma ser PowerShell e **quebra** estes comandos)

```bash
node -v   # esperado: v20.19+ ou v22.12+ (não 21.x)
npm -v
```
...
**Cwd canônico da trilha:** crie o projeto **dentro** de `backend/nest/` (ao lado do [`docker-compose.postgres.yml`](docker-compose.postgres.yml) e da pasta [`seed/`](seed/)).

> **Por que `@12`?** Esta trilha usa **NestJS 12** + TypeScript 6. No prompt do CLI, escolha **CJS (CommonJS) [with jest]** — **não** ESM/Vitest. Os pacotes `@nestjs/*` são ESM, mas o app CJS funciona via `require(esm)` no Node 20.19+/22.12+.

```bash
cd backend/nest          # pasta deste material
# Versão da trilha: NestJS 12 (CLI 12 — ex.: 12.0.3)
npx -y @nestjs/cli@12 new loja-api --package-manager npm --skip-git
```

No prompt do CLI: escolha **CJS (CommonJS) [with jest]** — NÃO ESM/Vitest.  
(`--package-manager npm` já fixa o gerenciador; `--skip-git` evita repo git aninhado dentro da trilha.)

Confira a major do scaffold:
...
> **Nota `tsconfig`:** o scaffold Nest 12 CJS costuma trazer `strictPropertyInitialization: false`. Não force `true` nesta trilha sem ajustar as entidades/DTOs.  
> **Nota Jest:** no [cap. 10](10.testes_software.md) você ajusta os scripts `test`/`test:e2e` para `node --experimental-vm-modules …` (pacotes `@nestjs/*` são ESM).
...
|---------------|---------|------|
| `backend/nest/` | `docker compose -f docker-compose.postgres.yml up -d` ([arquivo](docker-compose.postgres.yml)) | [`seed/seed-catalog.sql`](seed/seed-catalog.sql) (cap. 5) ou [`seed/seed.sql`](seed/seed.sql) (cap. 6+) |
| `backend/nest/loja-api/` | `docker compose -f ../docker-compose.postgres.yml up -d` | `../seed/...` ([pasta seed](seed/)) |
