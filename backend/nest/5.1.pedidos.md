...
> **Antes de mergulhar:** você já tem `Product` no Postgres e curls de `/products` passando? Se não, feche o [cap. 5](5.crud_nest_bd.md) primeiro.  
> **Neste episódio:** (1) entidades `Order`/`OrderItem` · (2) DTO com `ValidateNested` · (3) **uma** transação que grava pedido e baixa estoque · (4) curls.  
> **Pause e rode:** após a transação, `POST /orders` + confira `stock` no produto.  
> **Recuperação (30s):** o que `ParseIntPipe` evita no `:id`? Onde vive o catálogo agora?  
> **Windows:** curls com `curl.exe`; seed/catalog via `Get-Content … | docker exec` — [FAQ](FAQ-TRAVOU.md).  
> **Não precisa decorar tudo de uma vez:** copie entidade → DTO → service → controller; valide cada etapa com `npm run start:dev`.
...
> **Travou na transação?** Use só o `manager` dentro de `dataSource.transaction` (não misture `repository` solto) · `cascade: true` no `@OneToMany` · DTO com `@ValidateNested` + `@Type(() => …)` — FAQ linhas “detached” / “pedido sem itens”.
...
> **Lab, não produção:** `POST /orders` sem login neste capítulo é **provisório** (didática). No cap. 6 essas rotas passam a exigir Bearer — não leve o desenho público para um projeto real.

Modelo de dados (núcleo P):
...
stateDiagram-v2
    [*] --> OPEN: POST /orders
    OPEN --> CANCELLED: POST .../cancel (P)
    OPEN --> PAID: pagamento (A)
    CANCELLED --> [*]
    PAID --> [*]
```
...
sequenceDiagram
    ...
    Svc->>Tx: begin
    loop cada item
        Tx->>PG: findOne Product
        Tx->>PG: create OrderItem + unitPrice
        Tx->>PG: decrement stock
    end
    Tx->>PG: save Order + items
    Tx-->>Svc: commit (ou rollback se falhar)
```
...
> **Shell:** Git Bash abaixo. **PowerShell:** `curl.exe` no lugar de `curl` — [FAQ](FAQ-TRAVOU.md) · [CURLS-P](CURLS-P.md).  
> **Pause intermediário:** entidades + DTO + module compilando → só então a transação e estes curls.  
> **Seed:** depois destes pedidos, para resetar o lab use [`seed.sql`](seed/seed.sql) (não só `seed-catalog`).
...
**Contrato HTTP:** [MAPA — Cap. 5.1](MAPA-LINHAS-P-A.md) — em conflito, o mapa prevalece.  
`DataSource.transaction` / `EntityManager`: [REFERENCIA-NEST §5](REFERENCIA-NEST.md#sec-typeorm).
