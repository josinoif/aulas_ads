...
> **Shell:** Bash (Linux ou **Git Bash** no Windows) — mesmos curls do cap. 5.  
...
> **Seed agora:** só [`seed-catalog.sql`](seed/seed-catalog.sql) (ou o `POST /products` abaixo). **Não** rode [`seed.sql`](seed/seed.sql) ainda — a tabela `users` só existe no **cap. 6**. Reset completo (Ana/Cli) = na **6a**, depois da API subir com `User`.
