...
Pacotes de autenticação **pinados** para Nest 12. Sem pin, majors podem divergir do [VERSIONS.md](VERSIONS.md) e o lab quebra:

| Pacote | Versão da trilha | Papel |
|--------|------------------|--------|
| `@nestjs/jwt` | **12.0.2** | emitir / verificar JWT |
| `@nestjs/passport` | **12.0.0** | ponte Nest ↔ Passport |
| `passport` | **0.7.0** | middleware de estratégias |
| `passport-jwt` | **4.0.1** | estratégia Bearer JWT |
| `bcryptjs` | **2.4.3** | hash de senha (puro JS) |
| `@nestjs/config` | **12.0.0** | `JWT_SECRET` no `.env` (se ainda não instalou no cap. 5) |

```bash
npm install @nestjs/jwt@12.0.2 @nestjs/passport@12.0.0 passport@0.7.0 passport-jwt@4.0.1 bcryptjs@2.4.3 @nestjs/config@12.0.0
  ...
  imports: [
    ...
    JwtModule.registerAsync({
      ...
      useFactory: (config: ConfigService) => ({
        ...
        signOptions: {
          // ConfigService → string genérica; JwtModule tipa expiresIn como number | StringValue (ms)
...
> **Obrigatório:** use `PassportModule.register({ defaultStrategy: 'jwt' })` — **não** `PassportModule` solto no `imports`.  
> **`expiresIn`:** o cast tipado acima é o padrão do `loja-api` (Nest 12 / `@nestjs/jwt@12`).
