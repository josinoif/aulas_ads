...
> **Windows:** token com `Invoke-RestMethod` — [README](README.md); seed: [seed/README](seed/README.md). Travou? [FAQ](FAQ-TRAVOU.md).  
...
> **Shell:** Git Bash abaixo. **PowerShell:** seed com `Get-Content .\seed\seed.sql | docker exec -i loja-postgres psql -U loja -d loja`; token com `Invoke-RestMethod` — [CURLS-P](CURLS-P.md) · [FAQ](FAQ-TRAVOU.md).

**Opção A — seed oficial** (recomendado no lab): aplique [`seed/seed.sql`](seed/seed.sql) e confira com [`seed/verify-seed.sh`](seed/verify-seed.sh) / [`verify-seed.ps1`](seed/verify-seed.ps1).
...
| Seed + [`verify-seed`](seed/verify-seed.sh) / [`.ps1`](seed/verify-seed.ps1) | OK |
...
> **Shell:** Git Bash abaixo. **PowerShell:** `curl.exe` + token via `Invoke-RestMethod` — [CURLS-P](CURLS-P.md).
