...
> **Windows:** use `curl.exe`; paths locais (sem `/tmp`); ver bloco PowerShell nos curls.  
...
O Nest 10 (`@nestjs/platform-express`) **já traz** o Multer como dependência. **Não** rode `npm install multer` solto: isso pode instalar outra major (ex.: 2.x) e gerar conflito com a cópia do Nest.

Nesta trilha você só precisa dos **tipos** TypeScript e da pasta de uploads:

| Pacote | Versão da trilha | Papel |
|--------|------------------|--------|
| `@types/multer` | **1.4.12** | tipagem de `diskStorage` / `file` (dev) |

```bash
npm install -D @types/multer@1.4.12
mkdir -p uploads          # Git Bash / macOS / Linux
# PowerShell: New-Item -ItemType Directory -Force uploads
echo 'uploads/' >> .gitignore
```

O import `import { diskStorage } from 'multer'` continua igual — o módulo vem do Nest, não de um `multer` extra no seu `package.json`.
...
```bash
# a partir de loja-api/, se copiou a fixture (Git Bash / macOS / Linux):
mkdir -p fixtures
...
Copy-Item ..\fixtures\caneca.jpg .\fixtures\caneca.jpg -Force
...
curl.exe -s -X POST http://localhost:3000/products/1/image `
  -H "Authorization: Bearer $($ana.access_token)" `
  -F "file=@.\fixtures\caneca.jpg"
curl.exe -s -o .\caneca-out.jpg -w "%{http_code}`n" http://localhost:3000/products/1/image
```
