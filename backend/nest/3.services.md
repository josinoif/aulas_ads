...
> **Neste episódio:** (1) controller vs service · (2) `ProductsService` + DI · (3) mesmas URLs do cap. 2.  
> **Pause e rode:** após mover a lógica, os curls do cap. 2 ainda passam.  
> **Recuperação (30s):** quem valida o body — controller ou `ValidationPipe`?  
> **Windows:** mesmos curls com `curl.exe` — [FAQ](FAQ-TRAVOU.md).  
> **Linha P hoje:** refactor interno. **A** (outro módulo) no fim.
...
```mermaid
flowchart LR
    CatCtrl[CatalogController] --> CatSvc[CatalogService]
    CatSvc -->|DI| ProdSvc[ProductsService]
    ProdMod[ProductsModule<br/>exports ProductsService] --> CatMod[CatalogModule<br/>imports ProductsModule]
```
...
- DI / providers: [REFERENCIA-NEST.md §4](REFERENCIA-NEST.md#sec-di)  
