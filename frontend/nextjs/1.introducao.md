...
- **Server Components** (padrão): rodam no servidor, podem ser `async`, acessam banco/APIs/secrets, não enviam JS ao cliente.
- **Client Components** (`'use client'`): rodam no navegador, têm estado (`useState`), efeitos (`useEffect`) e event handlers (`onClick`).

Boas práticas:

1. Deixe as pages e layouts como **Server Components** sempre que possível.
2. Coloque `'use client'` no **menor componente interativo** possível.
3. Passe dados de Server → Client via **props serializáveis**.
4. Para estado global via Context, crie um Provider como Client Component e renderize-o no layout raiz.
